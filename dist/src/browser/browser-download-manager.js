...
exports.default = exports.BrowserDownloadLockManager = void 0;
...
class BrowserDownloadLockManager {
  constructor() {
  ...
  async ensureBrowserDownload(majorVersion, downloadFunction) {
    ...
    const lockFilePath = (0, _path.join)(BROWSER_PATH, `${LOCK_FILE_PREFIX}${majorVersion}.json`);
    try {
      await mkdir(BROWSER_PATH, {
        recursive: true
      });
  ...
  async createLockFile(lockFilePath, majorVersion) {
    ...
    await writeFile(lockFilePath, JSON.stringify(lockData), {
      flag: 'wx'
    });
  }
  ...
  async checkBrowserExists(majorVersion) {
    try {
      await access((0, _path.join)(BROWSER_PATH, `orbita-browser-${majorVersion}`));
      return true;
    } catch {
      return false;
    }
  ...
  async checkLockValidity(lockFilePath) {
    try {
      const lockContent = await _fs.promises.readFile(lockFilePath, 'utf8');
  ...
  async cleanupStaleLocks() {
    try {
      const files = await _fs.promises.readdir(BROWSER_PATH);
      const lockFiles = files.filter(file => file.startsWith(LOCK_FILE_PREFIX));
      for (const lockFile of lockFiles) {
        try {
          const lockFilePath = (0, _path.join)(BROWSER_PATH, lockFile);
...
exports.BrowserDownloadLockManager = BrowserDownloadLockManager;
var _default = exports.default = BrowserDownloadLockManager;