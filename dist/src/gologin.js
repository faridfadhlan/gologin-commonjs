...
class GoLogin {
  ...
  async getProfile(profile_id) {
    ...
    const profileResponse = await (0, _http.makeRequest)(`${_common.API_URL}/browser/features/${id}/info-for-run`, {
      method: 'GET'
    }, {
  ...
  async postFile(fileName, fileBuff) {
    ...
    await (0, _http.makeRequest)(this.storageGatewayUrl, {
      headers: {
        browserId: this.profile_id,
  ...
  getGologinPreferences(profileData) {
    ...
    const startupUrls = (profileData.startUrl || '').split(',').map(url => url.trim()).filter(url => url);
    const preferences = {
    ...
    if (this.browserMajorVersion >= this.newProxyOrbitaMajorVersion && profileData.proxy?.mode !== 'none') {
      ...
      if (profileData.proxy.username) {
        ...
        proxyServer += encodedPassword ? `${encodedUsername}:${encodedPassword}@` : `${encodedUsername}@`;
      }
  ...
  async createStartup(local = false) {
    ...
    await _fs.promises.rm((0, _path.join)(profilePath, 'Default', 'Sync Data'), {
      recursive: true
    }).catch(() => null);
    ...
    if (!prefFileExists) {
      ...
      await mkdir((0, _path.join)(profilePath, 'Default'), {
        recursive: true
      });
  ...
  async getTimeZone(proxy) {
    ...
    if (proxy && proxy.mode !== PROXY_NONE) {
      ...
      data = await (0, _http.makeRequest)(TIMEZONE_URL, {
        proxy: proxyUrl,
    ...
    if (proxy?.id) {
      ...
      await (0, _http.makeRequest)(`${_common.API_URL}/proxy/set_proxy_statuses`, {
        timeout: 13 * 1000,
        maxAttempts: 3,
        method: 'POST',
        json: statusBody
        ...
        token: this.access_token,
        fallbackUrl: `${_common.FALLBACK_API_URL}/proxy/set_proxy_statuses`
      }).catch();
    }
    return this._tz.timezone;
  }
  async getTimezoneWithSocks(params) {
    ...
    const checkData = await (0, _http.checkSocksProxy)(agent);
  ...
  async spawnBrowser() {
    ...
    if (this.vnc_port) {
      ...
      let params = [`--remote-debugging-port=${remote_debugging_port}`, '--password-store=basic', `--tz=${tz}`, `--lang=${this.browserLang}`, `--window-size=${this.resolution.width},${this.resolution.height}`, '--window-position=0,0', `--user-data-dir=${profile_path}`];
    ...
    if (this.waitWebsocket) {
      ...
      const data = await (0, _http.makeRequest)(`http://127.0.0.1:${remote_debugging_port}/json/version`, {
        json: true,
        maxAttempts: 30,
        retryDelay: 1000,
        method: 'GET'
      });
      debug('WS IS', (0, _utils2.get)(data, 'webSocketDebuggerUrl', ''));
      this.is_active = true;
      return {
        wsUrl: (0, _utils2.get)(data, 'webSocketDebuggerUrl', ''),
  ...
  async uploadProfileDataToServer() {
    ...
    const updateResult = await (0, _http.makeRequest)(`${_common.API_URL}/browser/features/profile/${this.profile_id}/update_after_close`, {
      json: body,
      method: 'POST',
      maxAttempts: 3,
      retryDelay: 2000,
      ...
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser/features/profile/${this.profile_id}/update_after_close`
    }).catch(e => {
      console.log(e);
      return e;
    });
  ...
  killBrowser() {
    ...
    try {
      ...
      console.error(error);
      (0, _sentry.captureGroupedSentryError)(error, {
        method: 'killBrowser',
  ...
  async getRandomFingerprint(options) {
    ...
    const fingerprint = await (0, _http.makeRequest)(url, {
      method: 'GET'
    }, {
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser/fingerprint?os=${os}`
    });
  ...
  async create(options) {
    ...
    const response = await (0, _http.makeRequest)(`${_common.API_URL}/browser`, {
      method: 'POST',
      json
    }, {
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser`
    });
    return response.id;
  }
  async delete(pid) {
    const profile_id = pid || this.profile_id;
    await (0, _http.makeRequest)(`${_common.API_URL}/browser/${profile_id}`, {
      method: 'DELETE'
    }, {
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser/${profile_id}`
    });
  }
  async update(options) {
    ...
    const response = await (0, _http.makeRequest)(`${_common.API_URL}/browser/${options.id}`, {
      json: profile,
      method: 'PUT'
    }, {
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser/${options.id}`
    });
  ...
  async writeCookiesToFile(cookies, isSecondTry = false) {
    ...
    try {
      ...
      if (!isSecondTry && (error.message.includes('table cookies has no column') || error.message.includes('NOT NULL constraint failed'))) {
        await db.close();
        await _fs.promises.rm(cookiesPaths.primary, {
          recursive: true,
          force: true
        }).catch(() => null);
        await (0, _cookiesManager.createDBFile)({
      ...
      (0, _sentry.captureGroupedSentryError)(error, {
        method: 'writeCookiesToFile',
        profileId: this.profile_id
      });
    } finally {
      db && (await db.close());
      await (0, _common.ensureDirectoryExists)(cookiesPaths.primary);
      await (0, _common.ensureDirectoryExists)(cookiesPaths.secondary);
  ...
  async stopRemote() {
    ...
    const profileResponse = await (0, _http.makeRequest)(`${_common.API_URL}/browser/${this.profile_id}/web`, {
      method: 'DELETE'
    }, {
      token: this.access_token,
      fallbackUrl: `${_common.FALLBACK_API_URL}/browser/${this.profile_id}/web`
    });
    if (profileResponse) {
      return JSON.parse(profileResponse);
    }
  ...
  async quickCreateProfile(name = '') {
    ...
    return (0, _http.makeRequest)(`${_common.API_URL}/browser/quick`, {
      method: 'POST',
      json: {
        os,
        osSpec,
  ...
  async profiles() {
    const profilesResponse = await (0, _http.makeRequest)(`${_common.API_URL}/browser/v2`, {
      method: 'GET'
    }, {
      token: this.access_token,
  ...
  async getNewFingerPrint(os) {
    ...
    const fpResponse = await (0, _http.makeRequest)(`${_common.API_URL}/browser/fingerprint?os=${os}`, {
      json: true,
      method: 'GET'
    }, {
      token: this.access_token,
